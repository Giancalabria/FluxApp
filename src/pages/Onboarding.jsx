...
export default function Onboarding() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    if (!user?.id) { setError('Necesitás estar autenticado.'); return; }
    if (!name.trim()) { setError('Ingresá tu nombre.'); return; }
    if (!currency) { setError('Elegí una moneda.'); return; }
    ...
    >
      <Card sx={{ maxWidth: 400, width: 1 }}>
        <CardContent sx={{ p: { xs: 2.5, sm: 3.5 } }}>
          <Typography variant="h5" fontWeight={800} color="primary" textAlign="center" gutterBottom>
            FluxApp
          </Typography>
          ...
          <LinearProgress
            variant="determinate"
            value={step === 1 ? 50 : 100}
            sx={{ mb: 3, borderRadius: 4, bgcolor: 'secondary.light', '& .MuiLinearProgress-bar': { bgcolor: 'primary.main' } }}
          />
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Stack spacing={2.5}>
                ...
                <Button
                  ...
                  sx={{ borderRadius: 2 }}
                >
                  {saving ? <CircularProgress size={24} color="inherit" /> : '¡Empezar!'}
                </Button>
                <Button
                  variant="text"
                  size="small"
