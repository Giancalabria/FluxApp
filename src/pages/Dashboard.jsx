...
export default function Dashboard() {
  ...
  const activeCurrency =
    userCurrencies[activeCurrencyIdx]?.currency_code ?? null;
  ...
  const byCategory = useMemo(() => {
    ...
    transactions.forEach((t) => {
      const name =
        t.category?.name ?? categoryMap.get(t.category_id) ?? "Sin categoría";
  ...
  const byAccount = useMemo(() => {
    ...
    transactions.forEach((t) => {
      const name =
        t.account?.name ?? accountMap.get(t.account_id) ?? "Sin cuenta";
      map.set(name, (map.get(name) ?? 0) + Number(t.amount || 0));
    });
    return Array.from(map.entries()).map(([name, value]) => ({ name, value }));
      ...
      >
        ...
        >
          <Box>
            ...
            <Typography
              variant="body2"
              sx={{ color: "rgba(242,239,233,0.75)", mt: 0.25 }}
            >
              Hola, {username} 👋
            </Typography>
          </Box>
        ...
        <Box>
          ...
          <Card
            sx={{
              ...
              borderRadius:
                activeCurrencyIdx === 0 ? "0px 36px 36px 36px" : "36px",
              border: "none",
